...
- It uses the same file names / in-memory variable names as the R app:
    - lvm_bg_geo : block-group GeoDataFrame
    - pop_ethnorace : population DataFrame
...
- KY_blck_grp_2022.shp : block group shapefile used to produce lvm_bg_geo.
  Required columns:
...
- pop_ethnorace.csv : population CSV used to build pop_map in the R app.
  Required columns:
...
- lvm_bg_geo (optional): must be a GeoDataFrame already loaded in memory and matching
  the contents of the shapefile.
