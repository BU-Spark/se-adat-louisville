...
class AdminIn(BaseModel):
    """Minimal payload for writing to the `admin` table.

    Accepts either snake_case or the following camelCase aliases:
      - session_id / sessionID (UUID)
      - created_at / createdAt (ISO 8601 timestamp with timezone)
    """
    session_id: UUID = Field(..., alias="sessionID", json_schema_extra={"example": "3fa85f64-5717-4562-b3fc-2c963f66afa6"})
    ...
    # Free-form city information. Keep it flexible: string or JSON blob is accepted.
    city_information: Optional[Any] = Field(None, alias="cityInformation", json_schema_extra={"example": "Louisville, KY"})

    model_config = {"populate_by_name": True}


@app.post("/api/admin", status_code=201)
async def create_admin(payload: AdminIn):
    """Insert a minimal admin record into the Supabase `admin` table.

    The endpoint accepts the two fields listed above (either snake_case or
    camelCase). It forwards the insert to Supabase using the service_role key
