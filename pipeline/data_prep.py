...
def main() -> None:
    ...
    lvm_bg_geo = gpd.read_file(
        "DHNA/data/gis/KY_Jefferson_BG_2023.shp"
    ).to_crs(4326)
    ...
    Jefferson_ma = gpd.read_file("DHNA/data/gis/Comp_Plan_Market_Areas.shp")

    # Debug: show columns
    print("Market Areas columns:", Jefferson_ma.columns.tolist())

    # Handle the market area name column (might be missing now)
    if 'market_are' in Jefferson_ma.columns:
        print("   - Found 'market_are', renaming to 'market_area'")
        Jefferson_ma = Jefferson_ma.rename(columns={'market_are': 'market_area'})
    elif 'market_area' not in Jefferson_ma.columns:
        # Create default names if column is completely missing
        print("   WARNING: No market area name column, using OBJECTID")
        Jefferson_ma['market_area'] = 'Area_' + Jefferson_ma['OBJECTID'].astype(str)

    # Select only needed columns
    Jefferson_ma = Jefferson_ma[["OBJECTID", "market_area", "geometry"]]

    print(f"   - Market Areas loaded: {len(Jefferson_ma)} areas")

    # Save to different location - DON'T overwrite original!
    Jefferson_ma.to_file(
        "DHNA/data/local_area.shp",  # Different filename!
    ...
    print("\n" + "="*70)
    print("UPLOADING TO SUPABASE (REFERENCE-BASED)")
    print("="*70)
    ...
    try:
        ...
        print("\n1. Uploading CSV files to Supabase Storage...")
    
        files_to_upload = [
            {
                'local_path': 'DHNA/data/LVM_Risk_Database.csv',
                'bucket_path': 'LVM_Risk_Database.csv',
                'file_type': 'risk_data',
                'description': 'Displacement risk assessment data for all block groups'
            },
            {
                'local_path': 'DHNA/data/pop_ethnorace.csv',
                'bucket_path': 'pop_ethnorace.csv',
                'file_type': 'population',
                'description': 'Population and ethnicity data by area and year'
            },
            {
                'local_path': 'DHNA/data/local_area.csv',
                'bucket_path': 'local_area.csv',
                'file_type': 'local_area',
                'description': 'Local area definitions and boundaries'
            },
            {
                'local_path': 'DHNA/data/Renthub_quarterly_rent.csv',
        ...
        for file_info in files_to_upload:
            ...
            try:
                ...
                try:
                    supabase.storage.from_('dhna-output-data').remove([bucket_path])
                except:
                ...
                # Upsert metadata to data_files table
                metadata = {
                    'file_name': file_name,
                ...
                existing = supabase.table('data_files').select('id').eq('bucket_path', bucket_path).execute()
            
                if existing.data:
                    # Update existing
                    supabase.table('data_files').update(metadata).eq('bucket_path', bucket_path).execute()
                    print(f"   ✓ Updated metadata for {file_name}")
                else:
                    # Insert new
                    supabase.table('data_files').insert(metadata).execute()
                    print(f"   ✓ Created metadata for {file_name}")
        ...
        # ========================================================================
        # STEP 3: Update dashboard stats (optional but useful)
        # ========================================================================
        ...
        try:
            # Risk summary
            if os.path.exists('DHNA/data/LVM_Risk_Database.csv'):
                risk_df = pd.read_csv('DHNA/data/LVM_Risk_Database.csv')
                risk_summary = {
                ...
                supabase.table('dashboard_stats').update({
                    'metric_value': risk_summary,
                    'last_updated': datetime.now().isoformat()
                }).eq('metric_name', 'risk_summary').execute()
            
                print(f"   ✓ Updated risk summary stats")
        
            # Population summary
            if os.path.exists('DHNA/data/pop_ethnorace.csv'):
                pop_df = pd.read_csv('DHNA/data/pop_ethnorace.csv')
                pop_summary = {
                    'total_records': len(pop_df),
                    'total_population': int(pop_df['pop'].sum()) if 'pop' in pop_df.columns else 0,
                    'latest_year': int(pop_df['data_yr'].max()) if 'data_yr' in pop_df.columns else 0,
                    'last_updated': datetime.now().isoformat()
                }
            
                supabase.table('dashboard_stats').update({
                    'metric_value': pop_summary,
                    'last_updated': datetime.now().isoformat()
                }).eq('metric_name', 'population_summary').execute()
            
                print(f"   ✓ Updated population summary stats")
        
            # Rent summary
            if os.path.exists('DHNA/data/Renthub_quarterly_rent.csv'):
                rent_df = pd.read_csv('DHNA/data/Renthub_quarterly_rent.csv')
                rent_summary = {
                    'total_records': len(rent_df),
                    'last_updated': datetime.now().isoformat()
                }
            
                supabase.table('dashboard_stats').update({
                    'metric_value': rent_summary,
                    'last_updated': datetime.now().isoformat()
                }).eq('metric_name', 'rent_summary').execute()
            
                print(f"   ✓ Updated rent summary stats")
        ...
        print("\n" + "="*70)
        print("✅ SUPABASE UPLOAD COMPLETE (REFERENCE-BASED)!")
